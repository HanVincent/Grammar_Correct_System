    ...
    def preparse(self, text):
        ...
        for sent in doc.sents:
            ...
            for token in self.parse(sent.text):
                ...
                if token.dep_ == 'ROOT':
                    result.append('|'.join([str(token.i), token.text, token.lemma_,
                                  token.norm_, token.pos_, token.tag_, token.dep_, "-1", children]))
                else:
                    result.append('|'.join([str(token.i), token.text, token.lemma_, token.norm_,
                                  token.pos_, token.tag_, token.dep_, str(token.head.i), children]))
            yield '\t'.join(result)


if __name__ == '__main__':
    parser = Parser()
    doc = parser.parse('your course leader will be available to help you .')
    for tk in doc:
        print(tk.text, tk.pos_, tk.tag_)
